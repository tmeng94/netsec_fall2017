...
def passThroughPrint(txt):
    print("(Pass-through) " + str(txt))
...
class Lock1stLayerProtocol(StackingProtocol):
    def __init__(self, higherProtocol=None):
        if higherProtocol:
            passThroughPrint("Initializing 1st pass-through layer on " + type(higherProtocol).__name__)
        super(Lock1stLayerProtocol, self).__init__(higherProtocol)

    def connection_made(self, transport):
        passThroughPrint("Connection made at 1st pass-through layer on " + type(self.higherProtocol()).__name__)
        self.transport = transport
        higherTransport = StackingTransport(transport)
        self.higherProtocol().connection_made(higherTransport)

    def data_received(self, data):
        passThroughPrint("Data received at 1st pass-through layer on " + type(self.higherProtocol()).__name__)
        self.higherProtocol().data_received(data)

    def connection_lost(self, exc):
        passThroughPrint("Connection lost at 1st pass-through layer on " + type(self.higherProtocol()).__name__)
        self.higherProtocol().connection_lost(exc)
        self.transport = None


class Lock2ndLayerProtocol(StackingProtocol):
    def __init__(self, higherProtocol=None):
        if higherProtocol:
            passThroughPrint("Initializing 2nd pass-through layer on " + type(higherProtocol).__name__)
        super(Lock2ndLayerProtocol, self).__init__(higherProtocol)

    def connection_made(self, transport):
        passThroughPrint("Connection made at 2nd pass-through layer on " + type(self.higherProtocol()).__name__)
        self.transport = transport
        higherTransport = StackingTransport(transport)
        self.higherProtocol().connection_made(higherTransport)

    def data_received(self, data):
        passThroughPrint("Data received at 2nd pass-through layer on " + type(self.higherProtocol()).__name__)
        self.higherProtocol().data_received(data)

    def connection_lost(self, exc):
        passThroughPrint("Connection lost at 2nd pass-through layer on " + type(self.higherProtocol()).__name__)
        self.higherProtocol().connection_lost(exc)
        self.transport = None
...
lock: set the lock to be locked.''')
    ...
    if args.option == "unittest":
        ...
        f = StackingProtocolFactory(lambda: Lock1stLayerProtocol(), lambda: Lock2ndLayerProtocol())
